...
void RemapLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
	...
	float x_,y_,x, y, wx, wy, w00, w01, w10, w11, v00, v01, v10, v11;
	int x0, y0, x1, y1;
	for ( int n = 0; n < N; n++ ) {
		for ( int c = 0; c < C; c++ ) {
			for ( int h = 0; h < H1; h++ ) {
				for ( int w = 0; w < W1; w++ ) {
					x_ = coords[((n*2+0)*H1+h)*W1+w];
					y_ = coords[((n*2+1)*H1+h)*W1+w];
					x = x_ + w;
					y = y_ + h;
					x0 = floor(x);
					y0 = floor(y);
...
void RemapLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
	...
	float x_,y_,x, y, wx, wy, v00, v01, v10, v11;
	float dx_,dy_, dv00, dv01, dv10, dv11;
	int x0, y0, x1, y1;
	for ( int n = 0; n < N; n++ ) {
		for ( int c = 0; c < C; c++ ) {
			for ( int h = 0; h < H1; h++ ) {
				for ( int w = 0; w < W1; w++ ) {
					...
					v11 = getvalue(Vb, x1, y1, c, n, C, W0, H0);
					// Gradients
					dx_ = (wy-1)*v00 - wy*v01 + (1-wy)*v10 + wy*v11;
					dy_ = (wx-1)*v00 - wx*v10 + (1-wx)*v01 + wx*v11;
					dv00 = (1-wx)*(1-wy);
					dv01 = (1-wx)*wy;
					dv10 = wx*(1-wy);
					dv11 = wx*wy;
					...
					if (propagate_down[1]) {
						...
						addvalue(b1_diff, w,  h,  0, n, 2, W1, H1, dx_ * top_diff[((n*C+c)*H1+h)*W1+w]);
						addvalue(b1_diff, w,  h,  1, n, 2, W1, H1, dy_ * top_diff[((n*C+c)*H1+h)*W1+w]);
					}
				}
			}
		} 
